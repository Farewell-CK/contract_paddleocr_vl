...
1. **安装依赖**（建议放在独立虚拟环境）：
   ...
   ```bash
   python -m pip install -U "paddleocr[doc-parser]" gradio pillow
   ```
...
2. **生成示例合同**：
   ...
   ```bash
   python applications/contract_ocr_vl/generate_synthetic_contracts.py --output-dir applications/contract_ocr_vl/data/synthetic_contracts
   ```

3. **运行一次合同解析**：

   ```bash
   python -m applications.contract_ocr_vl.app --input applications/contract_ocr_vl/data/synthetic_contracts/EN-001.md --output-dir ./ocr_outputs
   ```
